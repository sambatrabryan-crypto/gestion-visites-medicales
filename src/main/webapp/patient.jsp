...
<%@ page import="model.Patient, java.util.List, java.util.Map" %>
...
<%
    ...
    boolean openModal = (editPatient != null);
...
<div class="app-container">
    ...
    <header class="app-header">
        ...
        <div class="logo-icon"><i class="fas fa-user-injured"></i></div>
        <div>
    ...
    <div class="content-area">
        ...
        <div class="section-header">
            ...
            <button class="btn btn-primary" id="newBtn"><i class="fas fa-user-plus"></i> Nouveau patient</button>
        </div>
        ...
        <div class="appointment-list" id="appointmentList">
            ...
            %>
                <div style="text-align:center; padding:40px 0; color:var(--color-gray-400); background:white; border-radius:var(--radius-lg); border:1px dashed var(--color-gray-300);">
                    <i class="fas fa-user-injured" style="font-size:32px; margin-bottom:10px; display:block; opacity:0.5;"></i>
                    Aucun patient enregistré
                </div>
            <%
                } else {
                    for (Patient p : patients) {
                        String initiales = (p.getNom() != null && !p.getNom().isEmpty() ? p.getNom().substring(0,1) : "") +
                                            (p.getPrenom() != null && !p.getPrenom().isEmpty() ? p.getPrenom().substring(0,1) : "");
                        boolean isMale = "Masculin".equals(p.getSexe());
                        int nbVisites = (visiteCounts != null && visiteCounts.get(p.getCodepat()) != null) ? visiteCounts.get(p.getCodepat()) : 0;
            %>
            <div class="appointment-card"
                 data-nom="<%= p.getNom().toLowerCase() %>"
                 data-prenom="<%= p.getPrenom().toLowerCase() %>"
                 data-code="<%= p.getCodepat() %>">
                <div class="appointment-info">
                    <div class="avatar-md"><%= initiales.toUpperCase() %></div>
                    <div class="patient-details">
                        <h4><%= p.getNom() %> <%= p.getPrenom() %></h4>
                        <div class="sub">
                            <span><i class="fas fa-hashtag"></i> #<%= p.getCodepat() %></span>
                            <span><i class="fas fa-map-marker-alt"></i> <%= p.getAdresse() %></span>
                        </div>
                    </div>
                    <span class="badge <%= isMale ? "male" : "female" %>">
                        <i class="fas <%= isMale ? "fa-mars" : "fa-venus" %>"></i> <%= p.getSexe() %>
                    </span>
                </div>
                <div class="appointment-actions">
                    <a href="PatientServlet?action=edit&id=<%= p.getCodepat() %>" class="action-icon" title="Modifier"><i class="fas fa-edit"></i></a>
                    <button type="button" class="action-icon danger" title="Supprimer"
                            onclick="confirmDelete(<%= p.getCodepat() %>, <%= nbVisites %>, '<%= p.getNom() %> <%= p.getPrenom() %>')">
        ...
        <div id="noResultsMsg" style="display:none; text-align:center; padding:40px 0; color:var(--color-gray-400); background:white; border-radius:var(--radius-lg); border:1px dashed var(--color-gray-300);">
            <i class="fas fa-user-injured" style="font-size:32px; margin-bottom:10px; display:block; opacity:0.5;"></i>
...
<div class="modal-overlay <%= openModal ? "open" : "" %>" id="modalOverlay">
    <div class="modal-box">
        <div class="modal-header">
            <h3><i class="fas <%= (editPatient != null) ? "fa-user-edit" : "fa-user-plus" %>"></i>
                <%= (editPatient != null) ? "Modifier le patient" : "Nouveau patient" %>
            </h3>
            <a href="PatientServlet" class="modal-close"><i class="fas fa-times"></i></a>
        </div>

        <form action="PatientServlet" method="post">

            <% if (editPatient != null) { %>
                <input type="hidden" name="codepat" value="<%= editPatient.getCodepat() %>">
            <% } %>

            <div class="form-group">
                <label><i class="fas fa-id-card"></i> Nom</label>
                <input type="text" name="nom" required value="<%= (editPatient != null) ? editPatient.getNom() : "" %>">
            </div>

            <div class="form-group">
                <label><i class="fas fa-id-card"></i> Prénom</label>
                <input type="text" name="prenom" required value="<%= (editPatient != null) ? editPatient.getPrenom() : "" %>">
            </div>

            <div class="form-group">
                <label><i class="fas fa-venus-mars"></i> Sexe</label>
                <select name="sexe">
                    <option <%= (editPatient != null && "Masculin".equals(editPatient.getSexe())) ? "selected" : "" %>>Masculin</option>
                    <option <%= (editPatient != null && "Féminin".equals(editPatient.getSexe())) ? "selected" : "" %>>Féminin</option>
                </select>
            </div>

            <div class="form-group">
                <label><i class="fas fa-map-marker-alt"></i> Adresse</label>
                <input type="text" name="adresse" required value="<%= (editPatient != null) ? editPatient.getAdresse() : "" %>">
            </div>

            <div class="modal-actions">
                <a href="PatientServlet" class="btn btn-secondary">Annuler</a>
                <button type="submit" class="btn btn-primary">
                    <i class="fas <%= (editPatient != null) ? "fa-check" : "fa-save" %>"></i>
                    <%= (editPatient != null) ? "Modifier" : "Créer" %>
                </button>
            </div>
...
<script>
    ...
    document.getElementById('modalOverlay').addEventListener('click', function(e) {
        if (e.target === this) {
            window.location.href = 'PatientServlet';
        }
    });
    ...
    (function() {
        const messages = {
            added:   { type: 'success', icon: 'fa-circle-check', text: 'Patient ajouté avec succès !' },
            updated: { type: 'success', icon: 'fa-circle-check', text: 'Patient modifié avec succès !' },
            deleted: { type: 'success', icon: 'fa-circle-check', text: 'Patient supprimé avec succès !' }
        };
    ...
    (function() {
        ...
        input.addEventListener('input', function() {
            ...
            cards.forEach(card => {
                ...
                const match = nom.includes(term) || prenom.includes(term) || code.includes(term);
                card.style.display = match ? '' : 'none';
                if (match) visibleCount++;
            });

            noResults.style.display = (visibleCount === 0 && term.length > 0) ? 'block' : 'none';
            list.style.display = (visibleCount === 0 && term.length > 0) ? 'none' : 'flex';
        });
    })();
</script>
